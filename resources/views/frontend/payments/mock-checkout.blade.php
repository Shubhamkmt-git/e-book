...
@section('title', 'Easebuzz Payment Gateway Simulator')

@section('content')
<div class="min-h-[80vh] flex items-center justify-center py-12 px-4 bg-slate-950/40 relative">
    <!-- Ambient Background -->
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute top-1/4 left-1/2 -translate-x-1/2 w-96 h-96 bg-brand-600/15 rounded-full blur-3xl"></div>
    ...
    <div class="relative w-full max-w-lg bg-slate-900/90 border border-slate-700/70 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
        ...
        <div class="text-center space-y-2 border-b border-slate-800 pb-5">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold uppercase tracking-wider">
                ...
                <span>Easebuzz Test Simulator</span>
            </div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight font-roboto">Easebuzz Sandbox Checkout</h1>
        ...
        <div class="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-4.5 space-y-3">
            ...
            <div class="flex items-center justify-between">
                ...
                <span class="text-xs font-mono font-bold text-brand-300">{{ $purchase->transaction_id }}</span>
            </div>
            <div class="pt-2 border-t border-slate-700/50 flex items-center justify-between">
                ...
                <span class="text-xl font-extrabold text-emerald-400 font-roboto">₹{{ number_format((float) $purchase->amount, 2) }}</span>
            </div>
        </div>
        ...
        <div class="space-y-3">
            ...
            <form action="{{ route('payments.mock-process', $purchase) }}" method="POST">
                @csrf
                <input type="hidden" name="action" value="success">
                <button 
                    type="submit" 
                    class="w-full py-3.5 px-5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-sm font-bold shadow-lg shadow-emerald-950/30 transition transform active:scale-[0.99] flex items-center justify-center gap-2.5 cursor-pointer"
                >
                    <i class="fa-solid fa-circle-check text-base"></i>
                    <span>Simulate Successful Payment (Paid)</span>
                </button>
            </form>
        ...
        <!-- Help Notice -->
        <div class="p-3.5 rounded-xl bg-brand-950/40 border border-brand-800/40 text-xs text-brand-200/80 space-y-1">
            <div class="flex items-center gap-2 font-bold text-brand-300">
                <i class="fa-solid fa-circle-info"></i>
                <span>Development &amp; Testing Mode</span>
            </div>
            <p class="text-[11px] leading-relaxed text-slate-400">
                This sandbox simulator verifies the full purchase flow, auto-download generation, delivery email dispatch, and admin order logging without connecting to bank APIs.
            </p>
        </div>
