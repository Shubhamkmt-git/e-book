...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-5">
    @foreach ($categories as $category)
        ...
        >
            <!-- Icon Container (Muted Brand -> Softer White with Smooth Scale/Glow) -->
            <div class="w-14 h-14 rounded-2xl bg-brand-50/90 text-brand-600 border border-brand-100/80 group-hover:bg-white/20 group-hover:text-white group-hover:border-white/30 group-hover:backdrop-blur-xs flex items-center justify-center text-2xl mb-3.5 group-hover:scale-110 transition-all duration-300 ease-in-out shadow-2xs">
                <i class="{{ $category['icon'] }} transition-transform duration-300 ease-in-out group-hover:rotate-6"></i>
            </div>
