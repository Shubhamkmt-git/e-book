...
<div class="w-full space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="space-y-1">
            ...
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight font-roboto">Edit Category</h1>
            <p class="text-xs sm:text-sm text-slate-500">Update category details, icon, sort order, and status.</p>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 space-y-5">
                ...
                <div class="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs space-y-4">
                    ...
                    @php $currentIcon = old('icon', $category->icon ?? 'fa-solid fa-layer-group'); @endphp
                    ...
                    <div class="flex items-center gap-3 p-3 bg-brand-50 border border-brand-200/70 rounded-xl">
                        <div class="w-10 h-10 rounded-xl bg-brand-600 text-white flex items-center justify-center text-lg shadow-sm shrink-0">
                            <i id="icon-preview-i" class="{{ $currentIcon }}"></i>
                        </div>
                        <div class="text-xs">
                            <p class="font-bold text-slate-700">Selected Icon</p>
                            <p id="icon-preview-label" class="text-slate-500 font-mono text-[11px]">{{ $currentIcon }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-8 sm:grid-cols-10 md:grid-cols-12 gap-2 max-h-52 overflow-y-auto pr-1">
                        ...
                        <button type="button"
                            ...
                            class="icon-btn w-10 h-10 rounded-xl border-2 flex items-center justify-center text-base transition cursor-pointer
                                {{ $currentIcon === $icon['class'] ? 'border-brand-500 bg-brand-50 text-brand-600' : 'border-slate-200 bg-slate-50 text-slate-500 hover:border-brand-400 hover:bg-brand-50 hover:text-brand-600' }}"
                ...
                <div class="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs space-y-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="status" id="status" required class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 focus:bg-white text-slate-800 text-sm border border-slate-200 focus:border-brand-500 focus:outline-none transition font-medium cursor-pointer">
                            ...
                            <option value="{{ $key }}" {{ old('status', $category->status) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <label class="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:border-amber-300 hover:bg-amber-50/30 transition select-none">
                            ...
                            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $category->is_featured) ? 'checked' : '' }} class="w-4 h-4 rounded text-amber-500 border-slate-300 focus:ring-amber-400 cursor-pointer">
                    ...
                    <div class="pt-3 border-t border-slate-100 space-y-1.5">
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Metadata</p>
                        <div class="text-xs text-slate-500 space-y-1">
                            <div class="flex justify-between"><span>Slug</span><span class="font-mono font-semibold text-slate-700">{{ $category->slug }}</span></div>
                            <div class="flex justify-between"><span>Created</span><span class="font-semibold text-slate-700">{{ $category->created_at?->format('M d, Y') }}</span></div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    let isSlugManuallyEdited = {{ old('slug') || $category->slug ? 'true' : 'false' }};
