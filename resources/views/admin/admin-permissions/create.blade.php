...
@section('title', 'Add Admin Permission')
...
<div class="w-full space-y-6">
    ...
    <div class="space-y-1">
        ...
        <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight font-roboto">Add Admin Permission</h1>
        <p class="text-xs sm:text-sm text-slate-500">Register a new system permission for role authorization.</p>
    </div>

    <!-- Create Form Card -->
    <div class="bg-white border border-slate-200/90 rounded-2xl p-6 sm:p-8 shadow-2xs">
        <form action="{{ route('admin.admin-permissions.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <p class="text-[11px] text-slate-400">Auto-generated from title. Unique key for middleware and permission gates.</p>
                    @error('slug')
                        <p class="text-xs text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
