...
<div class="w-full space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="space-y-1">
            ...
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight font-roboto">Add CTA</h1>
            <p class="text-xs sm:text-sm text-slate-500">Create a new call to action section.</p>
        </div>
        <div class="flex items-center gap-3 shrink-0">
            ...
            <button type="submit" form="cta-form"
                ...
                <span>Create CTA</span>
            </button>
        </div>
    </div>

    <form id="cta-form" action="{{ route('admin.ctas.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5" novalidate>
        @csrf
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <textarea name="subtitle" id="subtitle" rows="3"
                                ...
                                class="w-full px-4 py-3 rounded-xl bg-slate-50 focus:bg-white text-slate-900 placeholder-slate-400 text-sm border border-slate-200 focus:border-brand-500 focus:outline-none transition font-medium resize-y @error('subtitle') border-rose-300 bg-rose-50/50 @enderror">{{ old('subtitle') }}</textarea>
            ...
            <div class="space-y-6 sticky top-6 self-start">
                ...
                {{-- Action Row --}}
                <button type="submit"
                        class="w-full px-6 py-3 rounded-xl bg-brand-600 hover:bg-brand-700 active:bg-brand-800 text-white text-sm font-semibold shadow-sm transition cursor-pointer flex items-center justify-center gap-2">
                    <i class="fa-solid fa-plus text-xs"></i> Create CTA
                </button>
            </div>
        </div>
    </form>
