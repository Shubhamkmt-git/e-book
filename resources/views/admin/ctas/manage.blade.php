...
<div class="w-full space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="space-y-1">
            <nav class="flex items-center gap-2 text-xs text-slate-500 font-medium">
                ...
                <span class="text-brand-600 font-semibold">Manage CTA</span>
            </nav>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight font-roboto">Manage CTA</h1>
            <p class="text-xs sm:text-sm text-slate-500">Configure the Call to Action section displayed on the homepage.</p>
        </div>
    ...
    <form id="cta-form" action="{{ route('admin.ctas.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6" novalidate>
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 space-y-5">
                ...
                <div class="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs space-y-4">
                    <div class="flex items-center gap-2.5 pb-3 border-b border-slate-100">
                        ...
                        <p class="text-xs text-slate-400 ml-1">Optional · Max 5 MB</p>
                    </div>
                    ...
                    <div id="image-drop-zone"
                        ...
                        <div id="image-placeholder" class="{{ $cta->bg_image ? 'hidden' : '' }} flex flex-col items-center gap-3 py-8 px-6 text-center">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-slate-600">Click or drag to upload</p>
                                <p class="text-xs text-slate-400 mt-0.5">JPEG, PNG, WebP, GIF</p>
                            </div>
                        </div>
                        <img id="image-preview"
                             src="{{ $cta->bg_image_url ?? '' }}"
                             alt="Preview"
                             class="{{ $cta->bg_image ? '' : 'hidden' }} absolute inset-0 w-full h-full object-cover rounded-xl">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <input type="hidden" name="remove_bg_image" id="remove_bg_image" value="0">
                    </div>
                </div>
                ...
                <div class="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs space-y-5">
                    <div class="flex items-center gap-2.5 pb-3 border-b border-slate-100">
                        ...
                        <h2 class="text-sm font-bold text-slate-800">CTA Content</h2>
                    </div>

                    <div class="space-y-1.5">
                        <label for="label" class="block text-xs font-bold uppercase tracking-wider text-slate-700">Label Tag</label>
                        <input type="text" name="label" id="label" value="{{ old('label', $cta->label) }}"
                            placeholder="e.g. Exclusive Reader Gift"
                            class="w-full px-3.5 py-3 rounded-xl bg-slate-50 focus:bg-white text-slate-900 placeholder-slate-400 text-sm border border-slate-200 focus:border-brand-500 focus:outline-none transition font-medium @error('label') border-rose-300 bg-rose-50/50 @enderror">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $cta->title) }}"
                            placeholder="e.g. Get 3 Free Bestseller E-Books Today"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="subtitle" id="subtitle" rows="3"
                            placeholder="A short compelling subtitle for the CTA section…"
                            class="w-full px-3.5 py-3 rounded-xl bg-slate-50 focus:bg-white text-slate-900 placeholder-slate-400 text-sm border border-slate-200 focus:border-brand-500 focus:outline-none transition font-medium resize-none @error('subtitle') border-rose-300 bg-rose-50/50 @enderror">{{ old('subtitle', $cta->subtitle) }}</textarea>
            ...
            <div class="space-y-5 sticky top-6 self-start">
                ...
                <div class="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-2xs space-y-5">
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700">Status</label>
                        <label class="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:border-brand-300 hover:bg-brand-50/30 transition select-none">
                            ...
                            <input type="checkbox" name="status" value="active"
                                   {{ old('status', $cta->status) === 'active' ? 'checked' : '' }}
                ...
                <div class="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-3">
                    ...
                    <div class="space-y-2.5 text-xs">
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-slate-700">#{{ $cta->id }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500 font-medium">Created</span>
                            <span class="font-bold text-slate-700">{{ $cta->created_at?->format('d M Y') ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500 font-medium">Updated</span>
                            <span class="font-bold text-slate-700">{{ $cta->updated_at?->format('d M Y') ?? '—' }}</span>
                        </div>
                    </div>
                </div>
...
function removeImage() {
    ...
    document.getElementById('remove_bg_image').value = '1';
}
